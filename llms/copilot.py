...
class CopilotLLM(OpenAILLM):
    ...
    def fetch_token(self):
        ...
        try:
            ...
            import os
            vscode_copilot_dir = os.environ.get("VSCODE_COPILOT_DIR", os.path.expanduser("~/vscode-copilot"))
    ...
    def tokenize(self, text: str) -> list[str]:
        """Basic tokenization - you may want to use a proper tokenizer"""
...
class CopilotClaudeLLM(CopilotLLM):
    ...
    def generate(self, messages, tools, **kwargs) -> LLMResponse:
        ...
        # }
        # find the first text message in the response
        text_messages = [
            r.message.content for r in response.choices if r.message.content
        ]
        text_message = text_messages[0] if text_messages else ""
