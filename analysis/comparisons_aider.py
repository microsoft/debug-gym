...
def compare_task_summary(base_summary, agent_summary):
    count_analysis = {
        "reduce rewrite": 0,
        "increase rewrite": 0,
        "worse than baseline": 0,
        "better than baseline": 0,
        "same as baseline": 0,
    }
    tool_analysis = {
        "reduce_rewrite_n_pdb_usage": 0,
        "reduce_rewrite_n_view_usage": 0,
        "task_reduce": 0,
        "better_baseline_n_pdb_usage": 0,
        "better_baseline_n_view_usage": 0,
        "task_better": 0,
    }
        ...
        if base_summary[task]["success"] == 1:
            if agent_summary[task]["success"] == 1:
                if base_summary[task]["rewrite"] >= agent_summary[task]["rewrite"]:
                    ...
                    tool_analysis["reduce_rewrite_n_pdb_usage"] += agent_summary[task][
                        "pdb"
                    ]
                    tool_analysis["reduce_rewrite_n_view_usage"] += agent_summary[task][
                        "view"
                    ]
            ...
            if agent_summary[task]["success"] == 1:
                ...
                tool_analysis["better_baseline_n_pdb_usage"] += agent_summary[task][
                    "pdb"
                ]
                tool_analysis["better_baseline_n_view_usage"] += agent_summary[task][
                    "view"
                ]
                tool_analysis["task_better"] += 1
            else:
                count_analysis["same as baseline"] += 1
    tool_analysis["reduce_rewrite_n_pdb_usage"] = (
        tool_analysis["reduce_rewrite_n_pdb_usage"] / tool_analysis["task_reduce"]
    )
    tool_analysis["reduce_rewrite_n_view_usage"] = (
        tool_analysis["reduce_rewrite_n_view_usage"] / tool_analysis["task_reduce"]
    )
    tool_analysis["better_baseline_n_pdb_usage"] = (
        tool_analysis["better_baseline_n_pdb_usage"] / tool_analysis["task_better"]
    )
    tool_analysis["better_baseline_n_view_usage"] = (
        tool_analysis["better_baseline_n_view_usage"] / tool_analysis["task_better"]
    )
    ...
    for agent in total_count_analysis:
        for key in total_count_analysis[agent]:
            if key not in output_count_analysis:
                output_count_analysis[key] = total_count_analysis[agent][key] / len(
        ...
        for key in total_tool_analysis[agent]:
            ...
            if key not in output_tool_analysis:
                output_tool_analysis[key] = total_tool_analysis[agent][key] / len(
                    total_tool_analysis
                )
            else:
                output_tool_analysis[key] += total_tool_analysis[agent][key] / len(
...
for baseline_name in subfolder_name_baseline:
    ...
    average_count_analysis[baseline_name], average_tool_analysis[baseline_name] = (
        average_analysis(total_count_analysis, total_tool_analysis)
    )
    print(average_count_analysis[baseline_name])
    print(average_tool_analysis[baseline_name])

final_count, final_tool = average_analysis(
    average_count_analysis, average_tool_analysis, std=False
)
# Create the plots
