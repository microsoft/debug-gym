...
class TooledEnv:
    ...
    def get_triggered_tools(self, action: ToolCall):
        ...
        if (
            action.id == "invalid_tool_response"
            and tool_name == "invalid_tool_response"
        ):
            error = (
                tool_kwargs.get("error")
                if isinstance(tool_kwargs, dict)
                else "The generated action was invalid"
            )
            return f"Invalid tool call: {error}", None
        if tool_name not in self._tools:
            # failed to find tool
