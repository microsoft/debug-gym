...
def aider_env(mock_open, mock_listdir, mock_read_text, mock_iterdir, mock_exists, mock_run):
...
@patch('subprocess.run')
def test_reset(mock_run, mock_load_dataset, mock_extract_reward, mock_extract_max_score, mock_cleanup, mock_load_current_file, mock_setup_workspace, mock_line_number, repo_env, aider_env):
    aider_env.dataset = {
        "test_task": {
...
def test_make_froggyignore(mock_listdir, mock_open, aider_env):
    aider_env.make_froggyignore("test_directory")
