...
def terminal_simulator_env(mock_open, mock_tempdir, mock_exists):
    ...
    env = TerminalSimulatorEnv(
        bug_free_code_path="/path/to/bug_free_code",
        buggy_code_path="/path/to/buggy_code.json",
    )
    return env
...
def test_load_dataset(mock_open, mock_exists, terminal_simulator_env):
    ...
    assert terminal_simulator_env.dataset["test_task"]["original path"] == [
        "path/to/file.py"
    ]
    assert terminal_simulator_env.dataset["test_task"]["new code"] == [
        'print("buggy code")'
    ]
